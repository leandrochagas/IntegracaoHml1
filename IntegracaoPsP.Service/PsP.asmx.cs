...
namespace IntegracaoPsP.Service
{
    ...
    public class PsP : WebService
    {
        ...
        [WebMethod(Description = @"Método responsável fornecer os registros que estão aptos a serem processados pelo terceiro")]
        public List<string>EnviarTxt(string token)
        {
            Utils util = new Utils();
            SqlConnection conn = util.ConexaoDb();
            conn.Open();
            ...
            try
            {
                if (token == "linkcon.com/2018")
                {
                    ...
                    List<Integration> lista = new List<Integration>();
                    lista = db.Integracoes.Where(x => x.Situacao == "Aguardando" && (x.Entidade =="Boletim" || x.Entidade=="Manifesto")).ToList();
                    List<string> xmls = db.Integracoes.Where(x => x.Situacao == "Aguardando" && (x.Entidade == "Boletim" || x.Entidade == "Manifesto")).Select(x => x.TxtRecebido).ToList();
                   
                    foreach (var item in lista)
                    {
                        util.UpdateIntegracao(item.Entidade, item.NomeArquivo, "Aguardando Retorno", item.IntegrationId);
                        util.InsereHistoricoIntegracao(item.Entidade, item.QtdeRegistros, item.NomeArquivo);                        
                    }
                    sqlTrans.Commit();
